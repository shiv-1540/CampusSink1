...
const updateStudent = async (req, res) => {
  const { id, name, email, prn, dept_id, year, phoneno, division } = req.body;
...
const getStudents = async (req, res) => {
  try {
    const [results] = await db.query(`
      SELECT u.name, u.email,u.phoneno, s.prn, s.division, s.year, d.name AS department
      FROM users u
...
const getTeachers = async (req, res) => {
  try {
    const [results] = await db.query(`
      SELECT u.name, u.email,u.phoneno ,t.prn,t.course_id, d.name AS department 
      FROM users u
